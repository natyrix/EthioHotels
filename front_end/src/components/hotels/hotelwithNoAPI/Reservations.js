...
export default function ResReservations() {
  const classes = useStyles();
  const { reservations, message, loading, error } = useStoreState(state => state.rec);
  const {getReservations} = useStoreActions(actions => actions.rec);
  ...
  const handleClickOpen = (res) => {
    setDialogContent(<Reservation reservation={res} handleClose={handleClose}/>);
  ...
  return (
    <div className=''>
      ...
      <ErrorMessage error='No reservation'/>
      }
    </div>
  );
}
