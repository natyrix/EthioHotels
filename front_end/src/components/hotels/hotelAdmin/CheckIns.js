...
export default function CheckIns() {
  ...
  const { checkins, message, loading, error } = useStoreState(state => state.hotel);
  const {getCheckIns} = useStoreActions(actions => actions.hotel);
  ...
  const handleClickOpen = (chkin) => {
    setChkIn(chkin);
    setDialogContent(<CheckIn checkin={chkin}/>);
