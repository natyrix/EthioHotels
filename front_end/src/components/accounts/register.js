...
import {registerHotel} from "../../redux";
...
function Register(props) {
  ...
  const handleRegister = (values) => {
    ...
    if(isHemail_valid && isHpno_valid && isClemail_valid && isclConPass_valid){
      ...
      console.log(hotel);
      console.log(clerk);
      props.registerHotel(hotel, clerk)
          ...
          <div className="card">
            <div className="card-body">
              ...
              <form onSubmit={handleSubmit(handleRegister)}>
                ...
                <br/>
                <label >Website: <sup className="text-danger">*</sup> </label>
                <input type="text" name="hotel_website" placeholder="Website" required
                       className="form-control"
                       ref={register({})}
                />
                <br/>
                <label >Hotel's API: <sup className="text-danger">*</sup> </label>
                <input type="text" name="hotel_api" placeholder="API" required
                       className="form-control"
                       ref={register({})}
                />
                <br/>
                ...
                <input type="password" name="clerk_conpass" placeholder="Confirm Password" required
                       ...
                       ref={register({})}
                />
                <span className="text-danger">{ clerk_con_pass }</span>
                <br/>
                <span className="text-danger">{ valErrs }</span>
                <br/>
                <span className="text-danger">{ props.registerState.message }</span>
                <br/>
                <button className="btn btn-block btn-primary" type="submit">Register</button>
              </form>
              <br/>
              <p>Success: {props.registerState.success}</p>
              <p>Message: {props.registerState.message}</p>
              <p>Error: {props.registerState.error}</p>
            </div>
          </div>
        </div>
      </>
...
const mapDispatchToProps = dispatch =>{
  return{
    registerHotel: (hotel, clerk) => dispatch(registerHotel(hotel, clerk))
  }
};
